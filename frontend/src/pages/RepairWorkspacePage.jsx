      ...
      <header className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-secondary text-lg">
            <span className="font-semibold text-text-primary">{repair.device?.brand} {repair.device?.modelName}</span>
              ...
              <div className="grid grid-cols-2 gap-6 mt-6">
                <div>
                  ...
                  {(user?.roleCode === ROLES.ADMIN || hasPermission(PERMISSIONS.REPAIR_EDIT)) ? (
                    ...
                    <p className="font-semibold text-text-primary">{repair.assignedTo?.fullName || repair.assignedTo?.name || <span className="text-muted italic">Unassigned</span>}</p>
                  )}
                </div>
                <div>
                  <label className="text-xs font-bold text-muted uppercase tracking-wider mb-1 block">Intake Date</label>
                  <p className="text-sm text-text-primary">{new Date(repair.createdAt).toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2 max-h-56 overflow-y-auto pr-1">
              {repair.statusLogs?.length > 0 ? repair.statusLogs.map((log, i) => (
                <div key={log.id} className="flex gap-3 p-2 rounded-md bg-secondary/40 border border-panel hover:bg-surface transition-colors">
